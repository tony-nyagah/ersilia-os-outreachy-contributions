...
def featurize_molecules(input_file, model_id, output_file=None):
    ...
    if smiles_col is None:
        raise ValueError(
            "Could not find a column containing SMILES strings in the dataset"
        )
    ...
    try:
        ...
        for i in range(0, len(smiles_list), batch_size):
            print(
                f"Processing batch {i//batch_size + 1}/{(len(smiles_list)-1)//batch_size + 1}..."
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Featurize molecules using an Ersilia model"
    )
    parser.add_argument(
        "--input", "-i", required=True, help="Input CSV file containing molecules"
    )
    parser.add_argument(
        "--model", "-m", required=True, help="Ersilia model ID for the featurizer"
    )
    parser.add_argument(
        "--output", "-o", help="Output CSV file to save the featurized data"
    )
